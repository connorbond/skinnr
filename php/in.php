<?php
	...
	// Curl - Get image from public (Amazon hosted) Minecraft server
	$ch = curl_init();
	...
	if ($hCode=='200') {
		...
		echo "<div class='topMsg'><button type='button' class='fa fa-arrow-left fa-lg reset prevBtn hovtip' onclick='getBase(lastUser);' title='Back to Previous Skin'></button> &nbsp;&nbsp; '<strong><span class='cuser sml'>".$username."</span></strong>' &nbsp;&nbsp; <button type='button' class='fa fa-refresh fa-lg reset prevBtn hovtip' onclick='getBase(&#39;".$username."&#39;);' title='Reload Skin: ".$username."'></button> </form></div> ";
		...
		echo '</div>&nbsp;&nbsp;&nbsp;&nbsp;';
		suggest(); 	// Add suggested user to bottom
		echo '<br /><br />';
	} else {
		// Return error message, instructions, and a suggestion of another user.
		echo "<div class='topMsg'>Sorry, <strong>'".$username."'</strong> is not a Minecraft user.</div><br>";
		echo"<p><img src='img/sad.png' width='80px' height='80px' /></p><br><h2>Search again</h2><p>or upload an image from &nbsp;<strong><i class='fa fa-desktop'></i> Your Computer</strong> or <a class=''><strong><i class='fa fa-dropbox fa-lg'></i> Dropbox</strong></a>.</p>";
		suggest();
		echo '<br /><br />';
	}

	//drawSkin("Div ID", "Div Class", "Container Class", "Pixels Wide(x)", "Pixels High(y)", "Offset X", "Offset Y");
	// Resource: http://php.net/manual/en/function.imagecolorsforindex.php
	function drawSkin($id, $class, $cClass, $px, $py, $ox, $oy){
		$i = 0;
		global $im;
		$out = "\n\t\t <div id='".$id."' class='".$cClass."'>\n\t\t";
		for($y=0;$y<$py; $y++){					// Loop Y  (rows/lines - down)
			$out .= "<div class='line'>";		// Start line
			for($x=0;$x<$px; $x++){				// Loop X (columns - across)
				$rgb = imagecolorat($im, $ox+$x, $oy+$y);	// Get colour from specific pixel (in RGB)
				$cols = imagecolorsforindex($im, $rgb);		// 
				$r = dechex($cols['red']);		// Convert Red in RGB to Hex value
				$g = dechex($cols['green']);	// Convert Green in RGB to Hex value
				$b = dechex($cols['blue']);		// Convert Blue in RGB to Hex value
				if (strlen($r) == 1) {			
					$r = "0" . $r;
				}
				if (strlen($g) == 1) {			// If Hex is only 1 character, prepend 0 (otherwise final Hex value is too short)
					$g = "0" . $g;
				}
				if (strlen($b) == 1) {
					$b = "0" . $b;
	...
	// Dr
	// DIFFERENCE: X value loop (see $x) is reversed, so it draws right to left instead. Simples. Remaining function stays the same.
	...
	// Pull in list of users (text doc), grab random line and draw on screen a link to load that user.
	function suggest(){
		$f_contents = file("../txt/users.txt");					// Get file to array
		$line = $f_contents[rand(0, count($f_contents) - 1)];	// Get random 'line'
		$line = substr_replace($line, "", -2);					// Remove last character (trailing space)
		$out = "<br /><br /><div class='suggest sml'>&nbsp;&nbsp;Why don't you try: <strong><u><span class='cur loadSuggest' onclick='showSpin(); saveLast(); getBase(&#34;".$line."&#34;)' title='Load ".$line."'>".$line."</span></u></strong> <span class='trySpin novis'><i class='fa fa-refresh fa-spin'></i></span></div><br><br><br>";
		echo $out;
	}
?>