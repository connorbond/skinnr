<?php
	...
	/* ------------------------------------------------
		DRAWING FUNCTION - FROM UPLOADED FILES
	------------------------------------------------- */
	...
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, 'http://dev.connorbond.co.uk/skinnr/uploads/'.$file_url);
	...
	if ($hCode=='200') {
		...
		echo '<div class="topMsg"><h4>Loaded Local Image</h4>';
		echo '<p>Source: http://dev.connorbond.co.uk/skinnr/uploads/'.$file_url.'</p></div>';
	...
	function suggest(){
		...
		$out = "<br /><br /><div class='suggest sml'>Why don't you try: <strong><u><span class='cur' onclick='saveLast(); getBase(&#34;".$line."&#34;)' title='Load ".$line."'>".$line."</span></u></strong></div><br><br><br>";
		echo $out;
	}
?>